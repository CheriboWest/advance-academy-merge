...
**Dev/deploy branch workflow:** for creating a dev branch, testing it (Vercel frontend preview + local backend), and merging to `main` → production, follow the `dev-branch-workflow` skill (`.claude/skills/dev-branch-workflow/SKILL.md`). Key fact: Vercel auto-previews every branch, but the backend only deploys `main` (Railway `hearty-nourishment/AdvanceAcademyTools`), so backend changes must be tested with `npm run dev:all` locally — a branch preview alone won't exercise them.

**Production incident triage:** when production is broken (cvadvance.com down/500/timeout, backend crash, a feature failing live, need to rollback), follow the `debug-production` skill (`.claude/skills/debug-production/SKILL.md`) — it pulls Vercel + Railway logs via MCP, matches known failure modes, mitigates (rollback/env), then fixes at root via a branch. Always confirm before any production-writing action.
