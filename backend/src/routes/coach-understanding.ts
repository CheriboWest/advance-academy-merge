...
export async function registerCoachUnderstandingRoutes(app: FastifyInstance) {
  app.post('/api/coach-understanding/generate', async (_request, reply) => {
    try {
      return await generateCoachUnderstanding();
    } catch (error) {
      const code =
  ...
  app.get('/api/coach-understanding/reports', async (_request, reply) => {
    try {
      return await listCoachReports();
    } catch (error) {
      _request.log.error(error);
  ...
  app.get<{ Params: { id: string } }>('/api/coach-understanding/reports/:id', async (request, reply) => {
    try {
      const report = await getCoachReport(request.params.id);
