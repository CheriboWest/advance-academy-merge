...
function computeFitWarning(
  ...
  const skillWords = new Set(
    topSkills.flatMap((s) => s.toLowerCase().split(/\s+/)),
  );
...
export async function generateRecruiterPitch(
  ...
  const warning = computeFitWarning(
    senderSignal.topSkills,
    recruiterData.activeRoles,
  );
...
${desiredRole.salaryExpectation ? `- Salary expectation: ${desiredRole.salaryExpectation}` : ''}`,
  ...
  // Post-processing: compress if over 100 words
