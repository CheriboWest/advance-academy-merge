...
import type { ViewName, PersonaId, InterviewStep } from '@/lib/types'
...
export function InterviewPrepView({ onNavigate }: InterviewPrepViewProps) {
    ...
    <div>
      ...
      <div className="bg-blue-50 rounded-xl p-4 mb-8 flex items-center gap-4">
        <div className="flex items-center gap-3">
          <div className="px-3 py-1.5 bg-blue-900 text-white rounded-lg text-sm font-medium">
            Text Mode
          </div>
          ...
          <div className="flex items-center gap-3 mb-4 p-3 bg-blue-50 rounded-xl">
            ...
            <div className="flex gap-1.5 ml-auto">
              {persona.style.split(' · ').map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-0.5 bg-white text-gray-600 text-xs rounded-full"
                >
                  {tag}
                </span>
              ))}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="space-y-2">
            ...
            {candidateAnswerCount > 0 && (
              <RunningAverages messages={session.messages} />
            )}
          </div>
        </div>
...
function RunningAverages({
  ...
  const avg = (fn: (s: NonNullable<(typeof scored)[0]['irsScore']>) => number) =>
    Math.round(
      (scored.reduce((sum, m) => sum + fn(m.irsScore!), 0) / scored.length) * 10
    ) / 10
  ...
  return (
    <div className="space-y-1.5 pt-2 border-t border-gray-100">
      ...
      <div className="flex justify-between text-xs font-semibold">
        ...
        <span className={irsScoreColor(avg((s) => s.overall))}>
          {avg((s) => s.overall).toFixed(1)}/10
        </span>
      </div>
    </div>
  )
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <div className="mt-4 flex items-center justify-center gap-2">
          <span
            className={`text-5xl font-bold tabular-nums ${irsScoreColor(report.overallIRS.overall)}`}
          >
          ...
          <div className="text-left">
            ...
            <p
              className={`text-sm font-semibold ${irsScoreColor(report.overallIRS.overall)}`}
            >
